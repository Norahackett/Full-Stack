...
suite("Trail API tests", () => {
  ...
  setup(async () => {
    ...
    lenisterTriallist = await trailtimeService.createTraillist(lenister);
  });

  teardown(async () => {});

  test("create trail", async () => {
    const returnedTrail = await trailtimeService.createTrail(lenisterTriallist._id, munster);
  ...
  test("create Multiple trails", async () => {
    for (let i = 0; i < testTrails.length; i += 1) {
      ...
      await trailtimeService.createTrail(lenisterTriallist._id, testTrails[i]);
    }
  ...
  test("denormalised traillist", async () => {
    ...
    const returnedTraillist = await trailtimeService.getTraillist(lenisterTriallist._id);
